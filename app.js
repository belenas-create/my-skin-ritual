...
function bindCloudAuth(){
 const signInBtn=$('#signInBtn'),signOutBtn=$('#signOutBtn'),badge=$('#userBadge'),avatar=$('#userAvatar'),nameEl=$('#userName');
 if(!signInBtn||!badge)return;
 signInBtn.onclick=()=>{
   signInBtn.disabled=true;
   signInWithGoogleAccount().catch(error=>{console.error(error);toast('Não foi possível entrar com o Google')}).finally(()=>{signInBtn.disabled=false});
 };
 if(signOutBtn)signOutBtn.onclick=()=>{
   if(confirm('Sair da sua conta? Os dados continuam salvos na nuvem e voltam a aparecer quando você entrar de novo.'))signOutOfCloud().catch(console.error);
 };
 window.addEventListener('skin-cloud-auth',event=>{
   const user=event.detail;
   if(user){
     signInBtn.hidden=true;
     badge.hidden=false;
     if(avatar){if(user.photo){avatar.src=user.photo;avatar.hidden=false}else{avatar.hidden=true}}
     if(nameEl)nameEl.textContent=user.name||user.email||'Conta conectada';
   }else{
